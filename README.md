...
- 🎓 **Guided tour** - an optional in-app walkthrough, offered once you've added your first printer, that points out each part of the app live: the connection bars and what they mean (including the "tunnel still building" marker you see just after pairing), the temperatures and the double-tap emergency stop, the webcam and preheat, and every option in the menu. Step forward or back, skip any time, and re-run it from the bottom of the menu.
...
- 🎨 **Themes & layout** - **Light / Dark / fully Custom** colours - with your own **dashboard background image** and adjustable **tile opacity** - plus a 1-3 column grid, font scaling, and optional landscape.
